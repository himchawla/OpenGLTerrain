...
public:
	...
	void Render(Camera* camera, GLuint program, glm::mat4 modelMat)
	{
		...
		mvpLoc = glGetUniformLocation(program, "model");

		glUniformMatrix4fv(mvpLoc, 1, GL_FALSE, glm::value_ptr(modelMat));

		mvpLoc = glGetUniformLocation(program, "cameraPos");

		glUniform3f(mvpLoc, camera->cameraPos.x, camera->cameraPos.y, camera->cameraPos.z);
		// EDIT END
