...
public:
	...
	glm::vec3 lightPos = glm::vec3(-2.0f, 6.0f, 3.0f);
	...
	Model(std::string path, Camera* camera){
		...
		// EDIT
		this->program = ShaderLoader::CreateProgram("Resources/Shaders/light.vs", "Resources/Shaders/light.fs");
		this->camera = camera;
		// EDIT END
	...
	Model(std::string path, Camera* camera, bool n) {
...
private:
	...
	GLint TextureFromFile(const char* path, string directory)
	{
		...
		glTexImage2D(GL_TEXTURE_2D, 0, LoadedComponents, width, height, 0, LoadedComponents, GL_UNSIGNED_BYTE, image);
